...
export const ExpenseForm = () => {
    ...
    const handleChangeDate = (value : Expense["date"]) => {
        setExpense({...expense, date: value || new Date()});    
    }
    ...
    const handleSubmit = (e : React.FormEvent<HTMLFormElement>) => {
        ...
        if((expense.amount - previousAmount) > remainingBudget) {
            setError("Limite de presupuesto alcanzado")
            return
        }
    ...
    const isEditing = () => {
        return state.editingId ? "Actualizar Gasto" : "Registrar Gasto"
    }
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <input 
            type="submit"
            className="bg-blue-600 cursor-pointer w-full p-2 text-white uppercase font-bold rounded-lg hover:opacity-75 transition-all"
            value={isEditing()}
        />
    </form>
  )
